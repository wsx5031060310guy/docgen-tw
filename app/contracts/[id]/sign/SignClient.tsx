...
import type { SignFetchData } from "@/lib/sign-fetch";
import { contractTitle, getTemplate, TEMPLATES } from "@/lib/templates";

type SignFetchError = { error: string; status: 404 | 400 };

type SignClientProps = {
  id: string;
  token: string;
  initial: SignFetchData | SignFetchError;
};
...
function SignInner({
  ...
  const [data] = useState<SignFetchData | SignFetchError>(initial);
  const initialData = "error" in initial ? null : initial;
  ...
  const [signed, setSigned] = useState(initialData?.fullySigned === true);
  const [signedAt, setSignedAt] = useState<string | null>(initialData?.recipientSignedAt ?? null);
  ...
  useEffect(() => {
    ...
    if ("error" in data || !documentElement || !documentBody) return;
  ...
  if ("error" in data) {
    if (data.status === 400 && data.error === "missing token") {
      return (
...
export default function SignClient({ id, token, initial }: SignClientProps) {
